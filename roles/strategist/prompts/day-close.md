...
- **WeekPlan:** /d/iwe/{{GOVERNANCE_REPO}}/current/WeekPlan W*.md (последний по дате)
- **MEMORY:** ~/.claude/projects/{{CLAUDE_PROJECT_SLUG}}/memory/MEMORY.md
- **Exocortex backup:** /d/iwe/{{GOVERNANCE_REPO}}/exocortex/
...
```bash
# Для КАЖДОГО репо в /d/iwe/:
git -C /d/iwe/<repo> log --since="today 00:00" --oneline --no-merges
```

- Пройди по ВСЕМ репозиториям в `/d/iwe/`
...
Скопируй актуальные файлы в `/d/iwe/{{GOVERNANCE_REPO}}/exocortex/`:

```bash
# Корневой CLAUDE.md
cp /d/iwe/CLAUDE.md /d/iwe/{{GOVERNANCE_REPO}}/exocortex/CLAUDE.md

# Memory (Слой 3)
cp ~/.claude/projects/{{CLAUDE_PROJECT_SLUG}}/memory/MEMORY.md /d/iwe/{{GOVERNANCE_REPO}}/exocortex/MEMORY.md
cp ~/.claude/projects/{{CLAUDE_PROJECT_SLUG}}/memory/*.md /d/iwe/{{GOVERNANCE_REPO}}/exocortex/
```
