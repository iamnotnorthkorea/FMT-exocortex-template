...
Показать пилоту 2-3 строки: **цифры** (сколько коммитов, файлов, репозиториев затронуто), не список сделанного текстом — та же логика, что у Дня, DP.D.288. Найдено живьём 31.07 (WP-484): агент дважды подряд подменял цифры прозой, хотя данные (репозитории, коммиты этой сессии) уже были под рукой — сам факт, что инструкция здесь называлась «свод», не гарантировал числа без явного слова «цифры». Если сессия длиннее 15 минут — раннер сам остановится на `session-reflection` (ai-контракт: вход `[session_summary]`, выход `[answer, skipped]`) и попросит сформулировать вопрос: «Что сегодня в этой сессии сработало / не сработало, что стоит запомнить?». Ответ пилота → `session-reflection-append` пишет в ledger. Сессия ≤15 мин → раннер пропускает вопрос сам (routing), ничего спрашивать не нужно.

### 3. «Ты свободен» [[gate]]

Сказать пилоту: **«Ты свободен, дальше довожу закрытие сам»** — и продолжить Часть Б (WP-context, KE, MEMORY.md, R23) без дальнейшего участия пилота, вызывая `process-runner.py next` по каждому оставшемуся шагу. Раннер уже это делает структурно (граф `quick-close.yaml`: после `session-reflection-append` идёт прямиком `session-index`, начало Части Б) — эта фраза просто явно произносится в чате, не только подразумевается.
...
3. **ТО памяти (T, SC.024.3)** — проверка здоровья статической нагрузки: [[narrative]]
   - `wc -l /d/iwe/.claude/rules/distinctions.md` → **> 80 строк = drift-флаг** (по правилу DP.KR.001 §6: 1-3 строки на различение). Предложить аудит в WP-7.
